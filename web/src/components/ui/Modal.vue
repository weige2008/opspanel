  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="open" class="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 overflow-y-auto"
           style="background: rgba(4,8,22,.45); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px);" @click.self="dismissable && emit('close')">
        <div class="glass-solid my-8 w-full rise" :class="wide ? 'max-w-3xl' : 'max-w-lg'">
          <div class="flex items-center justify-between px-6 py-4" style="border-bottom: 1px solid var(--border-soft);">
            ...
            <button v-if="dismissable" class="btn-icon btn-ghost" style="padding:6px;border-radius:8px" @click="emit('close')"><Icon name="x" :size="18" /></button>
          </div>
          <div class="px-6 py-5">
            <slot />
          </div>
